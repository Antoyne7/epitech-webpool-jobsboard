<template>
  <div>
    ...
    <b-container class="container-cards">
      ...
      <button v-show="showBtn" @click="showMore()" class="btn-show">
        Voir plus
      </button>
    </b-container>
  </div>
...
export default {
      ...
      [...this.listeOffresCopie].forEach(offre => {
        ...
        if (offre.typeoffres.length > 0) {
          ...
          offre.typeoffres.forEach(typeoffre => {
            ...
            if (
              (event.selectedTypeOffre == 0 ||
                typeoffre.id == event.selectedTypeOffre) &&
              !this.listeOffres.includes(offre)
            ) {
        ...
        if (
          ...
          event.location.code !== offre.code_ville &&
            event.location.code !== offre.code_departement
        ) {
        ...
        if (
          ...
          !offre.nom.toLowerCase().includes(event.query.toLowerCase()) &&
          this.listeOffres.includes(offre)
        ) {
          let index = this.listeOffres.indexOf(offre);
          this.listeOffres.splice(index, 1);
  ...
  computed: {
    showOffres() {
      this.listeToShow = [...this.listeOffres].filter(el => (el.pourvu === 0 || this.adminView)).slice(0, 6 * this.nbrShowed);
